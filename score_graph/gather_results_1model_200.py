"""
Script to gather the lowest scoring structural state from each replica of each replica exchange simulation for each snapshot model. Gathers the first 200 independent sampling runs.
Convergence of energies is tracked here, but not used (no filtering has occurred yet).
Refined energies are stored in output_dir. Path for main_dir needs to be specified.
"""
import sys
# global imports
import itertools
...
output_dir=main_dir+'Refined_energies_1model_200/'
...
for j in range(0,len(times)):
    ...
    # loop over all snapshot models at the time point
    for k in range(0,N_states):
        ...
        energy_list1=[]
        energy_list2=[]
        # loop over all independent sampling runs for that snapshot model
        for i in range(len(results_list)):
            # read in the log file and extract the energy
            results_dir=main_dir + results_list[i] +'/'+ time_dir + prefix
            print(results_dir)
            os.chdir(results_dir)
            # Loop over the number of replica exchange simulations
