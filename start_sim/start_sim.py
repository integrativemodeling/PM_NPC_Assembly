"""
Script that starts one round of initial simulations for each snapshot model. Only time points from 6 minutes to mature are run by this script.
...
code_dir='/.../code/'
results_dir=main_dir+'results200'
...
times=['6min','8min','10min','15min','mature']
N_sims={'6min':24,'8min':24,'10min':13,'15min':8,'mature':1}
...
def write_job(filename,config_file,seed,nslots):
    new=open(filename,'w')
    new.write('#!/bin/bash\n#$ -S /bin/bash\n#$ -cwd\n#$ -r n\n#$ -j y\n#$ -o log\n#$ -N p200m'+config_file[:-4]+'\n#$ -pe smp '+nslots+'\n#$ -l h_rt=168:00:00\n\n')
...
for i in range(0,len(times)):
    ...
    for j in range(0,N_states):
        ...
        # write job script
        job_file='job.sh'
        write_job(job_file,config_file,seed,mpi_np)
