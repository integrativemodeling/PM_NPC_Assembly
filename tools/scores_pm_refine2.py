...
def sterics(m, ps, k, slack, name="excluded_volume"):
    """
    Instantiate sterics.
    """
    ...
    #scnt = IMP.container.ListSingletonContainer(m, IMP.atom.get_leaves(ps))
    #cpc = IMP.container.ClosePairContainer(scnt, cutoff, IMP.core.QuadraticClosePairsFinder(), 0.0)
    #ev = IMP.core.ExcludedVolumeRestraint(cpc, k, slack, name)

    frag_list = []
    for frag in IMP.atom.get_leaves(ps):
            frag_list.append(frag)

    beads = IMP.container.ListSingletonContainer(m, frag_list)

    ev = IMP.core.ExcludedVolumeRestraint(beads, k, slack, name)
...
def initialize_restraints(m, hc, boundary, prm):
    ...
    if mdlc["sterics"]:
        ...
        ev = sterics(m,prot_sc.get_children(), gomp["sterics_k"], gomp["sterics_slack"])
        rs.append(ev)
        print(" Done.")
