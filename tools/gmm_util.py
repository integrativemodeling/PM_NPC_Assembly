...
def gmm2map_parallel(to_draw,voxel_size,bounding_box=None,origin=None, fast=False, factor=2.5):
    if type(to_draw[0]) in (IMP.Particle,IMP.atom.Hierarchy,IMP.core.Hierarchy):
        ps=to_draw
    elif type(to_draw[0])==IMP.core.Gaussian:
        ps=[g.get_particle() for g in to_draw]
...
def fit_gmm_to_points(points,
    ...
    if force_radii!=-1.0:
        gmm.covariances_=np.array([[force_radii]*3 for i in range(n_components)])
    if init_centers!=[]:
    ...
    if not gmm.converged_:
        print("whoa nelly")
        #raise Exception("GMM fitting failed to converge.")
